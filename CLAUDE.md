...
> **[plan/potenziamenti-diffusi.md](plan/potenziamenti-diffusi.md)** (gitignored). Branch:
> `feat/potenziamenti-diffusi`. Restano solo: merge in `main`, tag `v0.10.0` (GHCR), deploy NAS e
> le verifiche manuali a runtime (annotate nel piano e qui sotto). **Nessun lavoro di sviluppo
> aperto in questo batch.**
...
nota "solo SFW" aggiornata. +2 test (291) (`sketchy`→`purity=110`, default→`100`). **Step 17**
(Fase G — hardening + idee extra, scelta utente: tutto incluso + release; sub-step 17.1-17.11, un
commit ciascuno): **17.1** indice `idx_download_episode_file` (migrazione `0013`) per le azioni di
gruppo sulla coda gigante; **17.2** scoping dei full-table scan del gestore file (`trackedUnder`
range-prefix + binary-search `isManagedDir`); **17.3** fairness round-robin in `pickNext` (un nuovo
episodio non resta dietro tutta la coda One Piece, priorità preservata); **17.4** `library.unlinkExternal`
("Scollega esterno", chiude il limite Step 13); **17.5** redazione push keys (`p256dh`/`auth`) nei
log; **17.6** rate-limit per IP (60/min) sull'API in ingresso + fix `return` mancante sul 401;
**17.7** cooldown 6h sui falliti nell'auto-enqueue (retry reale via `worker.retry`, niente false
notifiche); **17.8** `nfo-service` sidecar `.nfo`+poster/fanart (config `writeNfo`); **17.9**
`jellyfin-service` refresh debounced + `testConnection` + card Impostazioni→Integrazioni; **17.10**
...
apre il full-res; nessuna regressione Setup wizard/mobile). **Step 17** Fase G (hardening + extra +
release, scelta utente "tutto incluso"). Sub-step con commit dedicati: **17.1** indice
`idx_download_episode_file` (migrazione `0013`) — le azioni di gruppo Step 8 (cancel/retry/groupItems)
risalivano `anime→episode→episode_file→download_queue` senza indice → scan di tutta la coda;
**17.2** `trackedUnder` (range-prefix BINARY `\u{10FFFF}`) + binary-search `isManagedDir` per non
caricare l'intera `episode_file` a ogni `list`/sync del gestore file; **17.3** `pickNext` fairness
round-robin (riepilogo per-anime `GROUP BY` O(#anime) + `lastServedAnime` in memoria; priorità
"Scarica prima" preservata; Regola #13 intatta); **17.4** `library.unlinkExternal`
(per-episodio/per-entry, guardia solo `external`, file mai toccati; UI bottoni "Scollega"); **17.5**
redazione `p256dh`/`keys.auth` nei log; **17.6** rate-limit per IP 60/min (hook `onRequest` prima
dell'auth, 429+Retry-After) su `/api/integration/requests` + fix `return` mancante sul 401;
**17.7** `addMissingImpl` con cooldown 6h sui falliti nell'auto-enqueue + retry **reale** via
`worker.retry` (prima `worker.enqueue` era no-op sulle righe esistenti → conteggio gonfiato → false
notifiche "Nuovi episodi"); **17.8** `nfo-service` (`tvshow`/`season`/`episode.nfo` + poster/fanart
best-effort dai metadati DB, idempotente, config `writeNfo`, hook `worker.on('complete')`);
**17.9** `jellyfin-service` (`testConnection` System/Info + `refresh` Library/Refresh debounced;
config `jellyfinServerUrl`/`jellyfinApiKey`[secret]/`jellyfinAutoRefresh`; router
`jellyfin.testConnection`; card "Jellyfin / Plex" in Impostazioni→Integrazioni col toggle `writeNfo`);
**17.10** scaffolding E2E Playwright (`playwright.config` webServer api+web mock, `e2e/smoke.e2e.ts`,
script `test:e2e`, job CI non bloccante; spec `*.e2e.ts` non raccolti da vitest; validato con
`--list`); **17.11** release v0.10.0 (CHANGELOG, bump versioni, questa sezione, merge/tag/deploy).
**+25 test (291→316)**, lint/typecheck/build web verdi a ogni sub-step. **Batch COMPLETO.** Restano
le verifiche manuali a runtime (annotate nel piano) e merge `main` + tag `v0.10.0` (GHCR) + deploy NAS.
